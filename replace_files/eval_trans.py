...
from Planning.generate_text_responses import you_think_generate
...
@torch.no_grad()        
def evaluation_transformer_test_modified(out_dir, val_loader, llm_planner, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, best_multi, clip_model, eval_wrapper, draw = True, save = True, savegif=False, savenpy=False) : 
    ...
    for batch in val_loader:
        ...
        word_embeddings, pos_one_hots, clip_texts, sent_len, pose, m_length, token, name = batch
        bs, seq = pose.shape[:2]
        num_joints = 21 if pose.shape[-1] == 251 else 22
        ...
        for clip_text in clip_texts:
            # print("------------------------ clip text --------------------")
            # print(clip_text)
            response_texts = you_think_generate(user_request=clip_text,**llm_planner)[1]
            # print("------------------------ response text --------------------")
            # print(response_texts)
            texts = clip.tokenize(response_texts, truncate=True).cuda()
            feat_clip_texts = clip_model.encode_text(texts).float()
            encoded_feat_clip.append(feat_clip_texts)

        # encoded_feat_clip = torch.stack(encoded_feat_clip)
        # print(f"encoded_feat_clip.shape = {encoded_feat_clip.shape}")
        ...
        for i in range(1):
            ...
            for k in range(bs):
                try:
                    index_motion = trans.sample(encoded_feat_clip[k], True)
                except:
                    index_motion = torch.ones(1,1).cuda().long()
